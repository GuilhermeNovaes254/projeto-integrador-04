...
const ativarBotaoDaBusca = () => {
  if (inputGames) {
    inputGames.addEventListener('change', function () {
      ...
      console.log('ativarBotaoDaBusca************')
      console.log(valueSearch)
    })

    btnSearch.addEventListener('click', function () {
      window.location.href = Url + "/busca?tipo=jogo&nomeUsuario=&nomeJogo=" + (typeof(valueSearch) !== "undefined" ? valueSearch : '') + "&tema=0&dominio=0&mecanica=0";
    });
  }
}
  ...
  if (boxOpcoes) {
    boxOpcoes.addEventListener('change', function () {
      ...
      if (targetId == 'search-form_content_games') {
        ...
        btnSearch.addEventListener('click', function () {
          window.location.href = Url + "/busca?tipo=usuario&nomeUsuario=" + (typeof(valueSearch) !== "undefined" ? valueSearch : '') + "&nomeJogo=&tema=0&dominio=0&mecanica=0";
        });
      }
